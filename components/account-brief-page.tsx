...
import { ArrowRight, Building2, Check, ExternalLink, Users } from "lucide-react";
    ...
    <main className="min-h-screen bg-[var(--paper)] text-[var(--ink)]">
      ...
      <section className="mx-auto max-w-[1480px] px-5 py-7 sm:px-8 lg:px-12 lg:py-10">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-[var(--rule)] pb-4">
          <div className="flex items-center gap-3"><span className="sample-label">Account brief</span><span className="text-sm text-[var(--muted-ink)]">Public-source research · internal report</span></div>
        ...
        <div className="brief-shell">
          <aside className="brief-rail">
            ...
            <div className="rail-action"><p className="eyebrow text-[var(--muted-ink)]">Recommended action</p><p className="mt-3 font-semibold leading-5">{brief.recommendation}</p><p className="mt-4 border-t border-[var(--rule)] pt-3 text-xs font-semibold uppercase tracking-[0.12em] text-[var(--muted-ink)]">Open checks are shown below</p></div>
          </aside>
          ...
          <article className="brief-body">
            ...
            <section className="brief-section"><div className="section-head"><div><p className="section-number">04</p><h3>Decision makers</h3></div><p>Current role and company match must be corroborated.</p></div><div className="contact-grid">{brief.decisionMakers.map((person) => <article className="contact-card" key={`${person.name}-${person.title}`}><div className="contact-top"><div className="contact-photo has-initials" aria-label={`${person.name} initials`}><span>{person.initials}</span></div><span className={`confidence-tag ${person.confidenceTone}`}>{person.confidence}</span></div><h4>{person.name}</h4><p className="contact-title">{person.title}</p><p className="contact-relevance">{person.relevance}</p><dl className="contact-facts"><div><dt>LinkedIn</dt><dd>{person.profileUrl ? <a href={person.profileUrl} target="_blank" rel="noreferrer">{person.linkedin}<ExternalLink /></a> : "No verified profile"}</dd></div><div><dt>Evidence</dt><dd>{person.evidence}</dd></div></dl></article>)}</div><div className="contact-rule"><p><strong>Contact rule</strong>Uncorroborated people stay out of the report and any outreach export. Multiple matches require manual review.</p></div></section>
